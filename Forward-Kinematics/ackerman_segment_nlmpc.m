...
% 在线段上创建参考目标点，避免超调
% 参考链接：http://www.cs.cornell.edu/courses/cs4620/2013fa/lectures/16spline-curves.pdf
...
for k = 1:Tsteps
    ...
    [uk, nlopt, info] = nlmpcmove(nlobj, xk, u_prev, y_ref(k:k+ph-1, :), [], nlopt);  % 使用 NL-MPC 控制器计算控制输入
    u_history(:, k) = uk;  % 存储控制输入
    u_prev = uk;  % 更新上一步的控制输入
    
    ODEFUN = @(t, xk) ackerman_dynamics(xk, uk, l, M1, M2);  % 定义微分方程
    [TOUT, YOUT] = ode45(ODEFUN,[0 Ts], xk');  % 使用 ODE45 求解微分方程
