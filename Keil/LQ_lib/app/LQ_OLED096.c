...
//龙邱OLED SPI口模块管脚顺序
#define OLED_CK    P64     //OLED CK管脚
#define OLED_DI	   P63     //OLED DI管脚
#define OLED_RST   P62     //OLED RST管脚
#define OLED_DC	   P61     //OLED DC管脚
#define OLED_CS	   P60     //OLED CS管脚 默认拉低，可以不用
...
{ 
   ...
   // OLED_CS=1;	//预制SLK和SS为高电平
    ...
    OLED_WrCmd(0x40);//--set start line address  Set Mapping RAM Display Start Line (0x00~0x3F)  设置开始行地址 集映射 RAM 显示开始行
    OLED_WrCmd(0x81);//--set contrast control register  设置对比度控制寄存器
    OLED_WrCmd(0xcf); // Set SEG Output Current Brightness  设置 SEG 输出电流亮度
    OLED_WrCmd(0xa1);//--Set SEG/Column Mapping     0xa0左右反置 0xa1正常
    OLED_WrCmd(0xc8);//Set COM/Row Scan Direction   0xc0上下反置 0xc8正常
    OLED_WrCmd(0xa6);//--set normal display  设置正常显示
    OLED_WrCmd(0xa8);//--set multiplex ratio(1 to 64)  设置多路复用比
    OLED_WrCmd(0x3f);//--1/64 duty
    OLED_WrCmd(0xd3);//-set display offset	Shift Mapping RAM Counter (0x00~0x3F)  设置显示偏移映射 RAM 计数器
    OLED_WrCmd(0x00);//-not offset  不偏移
    OLED_WrCmd(0xd5);//--set display clock divide ratio/oscillator frequency  设置显示时钟除法比/振荡器频率
    OLED_WrCmd(0x80);//--set divide ratio, Set Clock as 100 Frames/Sec   设置除法比，将时钟设置为 100 帧/秒
    OLED_WrCmd(0xd9);//--set pre-charge period  设置预充电周期
    OLED_WrCmd(0xf1);//Set Pre-Charge as 15 Clocks & Discharge as 1 Clock  将预充电设置为 15 个时钟和放电为 1 个时钟
    OLED_WrCmd(0xda);//--set com pins hardware configuration 设置 com 引脚硬件配置
    OLED_WrCmd(0x12);
    OLED_WrCmd(0xdb);//--set vcomh
    OLED_WrCmd(0x40);//Set VCOM Deselect Level 设置 VCOM 取消选择级别
...
{   
	...
	OLED_P6x8Str(19,2,"LQ e-motorcycle");		//字符串显示
	while (1)
	{  
	 OLED_P14x16Str(8,0,(unsigned char*)"北京龙邱智能科技");		//字符串显示	
		OLED_P6x8Str(19,2,"  LQ oled test  ");		//字符串显示
//		sprintf(txt,"CNT:%03d ",cnt++);  //		
